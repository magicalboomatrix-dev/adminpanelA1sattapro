...
export default function PremiumAdsEditor() {
    ...
    async function fetchAds() {
      try {
        ...
        if (Array.isArray(data)) {
          ...
          const top = normalized.filter(a => a.position === "top").sort((a,b)=>a.order-b.order);
          const middle = normalized.filter(a => a.position === "middle").sort((a,b)=>a.order-b.order);
          const bottom = normalized.filter(a => a.position === "bottom").sort((a,b)=>a.order-b.order);
  ...
  const handleSectionSave = async (position) => {
    ...
    try {
      ...
      // 🔥 BACKEND FIX: rely on returned ads
      if (Array.isArray(data.ads)) {
        ...
        alert(`${position.charAt(0).toUpperCase() + position.slice(1)} ads saved successfully!`);
      }
    } catch (err) {
  ...
  const renderAds = (position) => (
    <div className={`transition-all duration-300 ${activeSection === position ? 'block' : 'hidden md:block'}`}>
            ...
            >
              {ads[position].length === 0 ? (
                <div className="text-center py-10 border-2 border-dashed border-gray-300 rounded-xl bg-gray-50/50">
                  ...
                  <p className="text-gray-500">No {position} ads yet. Add your first one!</p>
                </div>
              ) : (
                      ...
                      {(provided, snapshot) => (
                        <div
                          ...
                          className={`
                            ...
                            ${snapshot.isDragging ? 'shadow-lg scale-[1.02] ring-2 ring-blue-500' : 'hover:shadow-md'}
                            overflow-hidden
                          `}
                        >
                          {/* Ad Header */}
                          <div className="flex items-center justify-between p-4 border-b bg-gradient-to-r from-gray-50 to-white">
                            <div className="flex items-center space-x-3">
                              <div {...provided.dragHandleProps} className="cursor-move p-2 hover:bg-gray-100 rounded-lg">
                                <FiMove className="w-4 h-4 text-gray-500" />
                              </div>
                              <div>
                                <h3 className="font-semibold text-gray-800">
                                  {position.charAt(0).toUpperCase() + position.slice(1)} Ad {index + 1}
                                </h3>
                          ...
                          <div className="p-3 border-b bg-gray-50/50">
                            <div className="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-10 gap-2">
                              ...
                              <ToolbarButton 
                                ...
                                label="Strike" 
                                onClick={() => execCommand(safeId, "strikeThrough")}
                              />
                              <ToolbarButton 
                              ...
                              <ToolbarButton 
                                icon={FiAlignLeft} 
                                label="Left" 
                                onClick={() => execCommand(safeId, "justifyLeft")}
                              />
                              <ToolbarButton 
                                icon={FiAlignCenter} 
                                label="Center" 
                                onClick={() => execCommand(safeId, "justifyCenter")}
                              />
                              <ToolbarButton 
                                icon={FiAlignRight} 
                                label="Right" 
                                onClick={() => execCommand(safeId, "justifyRight")}
                              />
                              <ToolbarButton 
                                icon={FiList} 
                                label="List" 
                                onClick={() => execCommand(safeId, "insertUnorderedList")}
                              />
                            </div>

                            {/* Image Size Controls */}
                            <div className="mt-3 flex flex-wrap items-center gap-2">
                              <span className="text-sm text-gray-600 font-medium">Image Size:</span>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {['top', 'middle', 'bottom'].map((position) => (
            <div key={position} className="bg-white rounded-2xl shadow-lg border overflow-hidden">
              {/* Section Header */}
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    <div className="p-2 rounded-lg bg-blue-100 text-blue-600">
                      ...
                      {position === 'middle' && <FiMaximize className="w-5 h-5" />}
                      {position === 'bottom' && <FiDownload className="w-5 h-5" />}
                    </div>
                    <div>
                      <h2 className="text-lg font-bold text-gray-900">
                        {position.charAt(0).toUpperCase() + position.slice(1)} Ads
                      </h2>
              ...
              <div className="p-5">
                ...
                >
                  <FiPlus className="w-5 h-5" />
                  Add New {position.charAt(0).toUpperCase() + position.slice(1)} Ad
                </button>
              </div>
            </div>
          ))}
            ...
            <button
              onClick={() => {
                if (confirm("Save all sections?")) {
                  ['top', 'middle', 'bottom'].forEach(pos => handleSectionSave(pos));
                }
              }}
              disabled={isSaving}
